...
| M1+M2 (합산) | 0.7520 | 71.3% | 0.7811 | 0% |

> Falco가 탐지하지 못한 공격에서 LSTM-AE 단독 탐지
> 
> **시나리오별 성능 차이 원인**
> 
> - **Scenario 5 (리버스쉘, 95.9%)** : 공격 시 syscall 분포가 정상과 크게 달라 복원 오차가 높게 나타남
> - **Scenario 4 (크립토마이닝, 73.8%)** : 채굴 과정의 syscall이 정상 패턴과 유사하여 복원 오차 차이가 작음
> - **M1+M2 (71.3%)** : M1 데이터가 약 6,800행으로 전체 학습 데이터(약 370만 행) 대비 0.2% 수준으로 부족하여 M1 성능이 전체 수치를 낮춤

### 실제 환경 데이터 — M1 / M2 클래스별
...
> M1은 약 6,800행으로 학습 데이터(약 370만 행) 대비 0.2% 수준으로 데이터 부족
